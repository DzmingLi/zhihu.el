...
在 Emacs 中使用 Org 或 Typst 作为源文件来撰写、存档并发布知乎
回答和文章，并可以指定文章专栏。
...
写新文章无需专用的新建命令：创建 `.typ` 或 `.org`
源稿即可。保留空 `article-id` 时作为新文章；文章需要文档标题。
需要指定专栏、话题或其它发布设置时，
再添加相应 metadata。
...
Org 和 Typst 文件；需要编辑辅助时手动运行 `M-x zhihu-mode`。
在 Typst `<zhihu>` dictionary 尚未写入冒号的直接字段名位置运行
`M-x completion-at-point`，
会按当前稿件类型和已有字段补全 `question-id:`、`article-id:` 等合法键名，
并排除重复字段。在 `column-id` 值槽运行
`M-x completion-at-point`，候选会显示当前账号可投稿的专栏名称和 ID，
选择后只把实际 ID 写入源稿。两种格式的 `column-id` 值槽和 `topics`
字符串元素也支持同样的就地补全。
...
Org 把它放在 `#+begin_export html` / `#+end_export` 中；Typst 可用
`html.elem` 输出同样的结构。博客
...
在 Org 或 Typst 源稿中运行 `M-x zhihu-insert-user-mention`，搜索并选择用户。
命令会插入当前格式的持久化链接标记，发布时转换为知乎原生
`member_mention` 链接。普通用户主页链接不会自动升级为 mention。
...
回答不支持 `topics`。源稿只保存话题名称；发布时再从知乎的名称完全匹配
候选取得对应 ID。
...
每篇源稿必须且只能包含 `question-id` 或 `article-id` 之一，分别
表示回答或文章。类型按字段是否存在判定，而不是按 ID 是否非空判定；
即使 ID 槽仍为空也算存在。

`article-id` 是唯一的空值例外：首次发布前保留空槽，发布成功后
自动在原位写入服务端 ID。Org 写作空值，Typst 写作 `none`。尚未取得的
`answer-id` 仍应整个省略。

需要加入专栏时，在含 `article-id` 的文章 metadata 下额外填写 `column-id`；
`column-id` 不能单独标记文章。发布文章后会检查当前专栏，尚未收录时才发起
收录。Org 和 Typst 使用相同字段。启用 `zhihu-mode` 后，可在两种格式的该字段
值槽调用 `completion-at-point`，按专栏名称选择并写入 ID；候选列表在当前
buffer 中懒加载并缓存，revert 后重新读取。

`banner` 与 `title` 一样属于通用文档 metadata：Org 使用 `#+BANNER:`，Typst
使用 `#metadata("...") <banner>`；它不写进 `<zhihu>` dictionary。值是非空的
本地图片路径，相对路径以源稿所在目录为基。
知乎文章发布会把它用作题图，字段缺失会清除知乎上的现有封面；回答发布
不会读取或使用它。

目录请求也属于通用文档结构，不属于知乎渠道字段。Org 使用
`#+TOC: headlines N`；Typst 使用原生 `#outline()`，也可用
`#outline(depth: 2)` 限制深度。只有文章
发布会读取目录请求，并把它映射为知乎原生文章目录；回答会忽略它。
...
在发布正文中。没有目录请求时，文章目录关闭。

以下知乎设置都属于单篇稿件。两种源格式使用表中的 metadata 字段；Org
把字段名大写并改用下划线，例如 `#+ZHIHU_COMMENT_PERMISSION:`。

| 设置 | Typst 字段 | 可用值 | 未填写时 |
| --- | --- | --- | --- |
| 创作声明 | `creation-statement` | `spoiler`、`medical_advice`、`fictional_creation`、`contain_finance`、`ai_creation` | 无创作声明 |
| 内容来源 | `content-source` | `officialWebsite`（官方网站）、`newsReport`（新闻报道）、`TVMedia`（电视媒体）、`printMedia`（纸质媒体） | 不标注来源 |
| 话题 | `topics` | 文章至多三个；回答不支持 | 文章会清空远端话题 |
| 转载权限 | `reprint-permission` | `allowed`、`disallowed`、`need_payment` | 使用转载权限默认值 |
| 评论权限 | `comment-permission` | `all`、`censor`、`followee`、`nobody` | 使用评论权限默认值 |
